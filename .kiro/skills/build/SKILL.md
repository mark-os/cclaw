---
name: build
description: Plan-then-execute implementation against SPEC.md. On test or build failure, auto-invokes backprop before retrying. Triggers when user asks to build, implement, execute the spec, or tackle a §T task. Expects SPEC.md to exist; if not, defers to spec skill.
---
...
`reference/pi`, `reference/openclaw`, and `reference/nullclaw` (Zig clone of OpenClaw) are available. When in doubt, steal ideas from them — but implement the simplest minimal version in C. Read their code to understand patterns, then strip to essentials.

## LOAD

1. Read `SPEC.md`. If missing → tell user to invoke the spec skill first. Stop.
2. Read `.kiro/skills/spec/FORMAT.md` once if not loaded.
3. Parse invocation args:
   - `§T.n` → that task only
   - `--all` → every `.` row in §T order
   - empty or `--next` → lowest-numbered row with status `.` or `~` (one task only)
...
Show plan. If the user said "don't prompt me" (or similar), skip confirmation and proceed immediately. Otherwise wait for user OK.
...
- Commit and push after each §T completes. Message: `T<n>: <goal line>` + §V cites.
