---
name: check
description: Read-only drift detector. Diffs SPEC.md against current code and reports violations grouped by severity. Writes nothing — suggests remedies via spec or build skills. Triggers on "check drift", "audit the spec", "does the code still match §V", "check invariants", "spec vs code".
---
