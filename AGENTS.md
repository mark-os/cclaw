...
Two wire formats are implemented (`src/llm_payload.c`): OpenAI-compatible chat completions (OpenRouter, OpenAI, DeepSeek, Cerebras, …) and Gemini-native `generateContent` (the direct Gemini provider — its OpenAI-compat endpoint is not used). Anthropic-native is not implemented; reach Claude models via OpenRouter. See [specs/providers.md](specs/providers.md). Switch provider via the `providers` table (or an approval-gated `request_config` changes document with a `provider` section — see [specs/config-doc.md](specs/config-doc.md)); API keys are stored encrypted in the `secrets` table (scope `system`) via `save_secret`, referenced by name (`api_key_env`). Env var `OPENROUTER_API_KEY` is all you need to start.
