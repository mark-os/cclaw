...
V73: ∀ DB access → single `cclaw.db` file (WAL mode, busy_timeout ≥ 5000ms); contains all tables: agents, agent_config, providers, kv, sessions, entries, tool_calls, inbox, memory_blocks, channels, channel_events, channel_outbox, channel_state, cron_jobs; parent (CLI/daemon) is primary writer; agent child opens same file via `CCLAW_DB` env var
