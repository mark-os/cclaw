...
V22: ∀ agent process → namespace sandbox at fork: `unshare(CLONE_NEWUSER|CLONE_NEWNS)` — `/` remounted ro, agent dir rw; graceful fallback if namespaces unavailable (log warning, continue without)
...
T210|.|namespace sandbox for shell_exec — `unshare(CLONE_NEWUSER\|CLONE_NEWNS\|CLONE_NEWNET)`, remount / ro, workspace rw, uid/gid maps; replace landlock code|V82,V37
T211|.|credential proxy thread — civetweb or raw sockets; MITM CA generation (per-process); allowlist check; credential injection by host+path; `SO_ORIGINAL_DST` for transparent mode|V83
T212|.|credential_mappings table in daemon.db + admin approval flow for new mappings; daemon reads at fork, passes to agent|V84,V54
T213|.|iptables REDIRECT in child netns — after unshare(NEWNET), set up loopback + veth pair to host proxy; redirect all TCP to proxy port|V82,V83
T214|.|daemon startup namespace check — read max_user_namespaces, test unshare, enforce agent limit ≤ max_ns/6, warn if constrained|V85
T215|.|per-process CA generation — ephemeral X509 CA + key in memory; write ca.pem to /tmp/cclaw-<pid>/; set SSL_CERT_FILE in shell child env; destroy on process exit|V82,V83
T216|.|test: shell curl through proxy with injected credentials — mock HTTPS server, verify auth header arrives without shell having credential in env|V82,V83
T217|.|test: shell cannot reach unlisted host — attempt connect to non-allowlisted host, verify proxy denies|V83
T218|.|test: shell cannot read filesystem outside workspace — verify /etc/shadow, agent.db, daemon.db all inaccessible from shell child|V82
T219|.|DNS interception in child netns — redirect UDP 53 to proxy; proxy resolves via system resolver, caches domain→IP; TCP handler recovers domain from cache for allowlist check; blocks hardcoded IPs by default|V86
