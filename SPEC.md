...
T164|x|split-column schema migration — recreate entries table; INSERT-SELECT from old table extracting `json_extract(data,'$.role')` → role int, `json_extract(data,'$.content')` → content text, tool_calls array, etc.; retain `data` column as nullable for rollback; FTS5 rebuild on `content`|V60,§D
