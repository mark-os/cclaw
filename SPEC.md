...
T299|.|extract `cclaw-llm` child — subcommand `cclaw llm --session=N`; inherits stdout (token stream); stderr piped to parent (journal); owns context planning + request build + curl + SSE parse + DB write; parent forks+waits+reads DB|T295,T296
