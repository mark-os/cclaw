...
T300|.|parent turn loop — fork `cclaw-llm` → wait → read DB → fork tools serially → wait → capture stdout pipe → write result to DB → loop until no tool_calls; CLI + daemon both use this orchestrator|T299
