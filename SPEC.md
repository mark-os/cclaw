...
T292|.|cache_control config — `cache_hints` values: `auto` (default, send top-level + per-block for all providers), `on` (force), `off` (disable), `gemini-native` (use T291 cachedContents API directly); env `CCLAW_CACHE_HINTS`|V90
