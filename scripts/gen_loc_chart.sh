#!/bin/bash
# Generate an SVG chart of lines of code (src/*.c, src/*.h, include/*.h) over commits.
set -eu
...
git log --reverse --format="%H" | {
    ...
    while read -r hash; do
        i=$((i+1))
        delta=$(git diff --numstat "$hash~" "$hash" -- 'src/*.c' 'src/*.h' 'include/*.h' 2>/dev/null | awk '{s+=$1-$2} END{print s+0}')
        delta=${delta:-0}
        total=$((total+delta))
        echo "$i,$total"
    done
