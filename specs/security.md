...
**Env injection is scoped to referenced secrets.** A shell child receives `CCLAW_SECRET_<NAME>` only for the secrets whose `$CCLAW_SECRET_<NAME>` token actually appears in the command (`command_uses_secret_env()`). Secrets used via `{{SECRET:NAME}}` are already interpolated into the command and get no env var. This is least-privilege: without it, every command would carry the agent's entire scoped credential set, and a single `cat /proc/self/environ` would dump them all.
