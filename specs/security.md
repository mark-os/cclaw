...
Grants bind directories only (`bind_dir_into()` mkdirs its mount target), so naming the DB file itself in a grant does not mount it. The DB is therefore masked unconditionally alongside the key. Nothing is lost: `db_query` is the sanctioned read path and runs in the trusted parent, so masking the file never affects it. If file-granularity grants are added, the DB rule is worth revisiting — it is ordinary agent state, and an exact, deliberate grant on it would be reasonable to honor. The key never is: reading it is not access to a resource but escalation to every secret in the store, including system-scope entries and other agents', so no grant may unmask it.
