# Trust Levels — Per-Agent Sandbox Profiles (as built)

`agents.trust_level` selects a **sandbox policy bundle** — containment only.
...
Resolution happens in ONE place: `sandbox_policy_from_trust()`
...
is the explicit opt-out, per-agent via `agents.trust_level` or session-wide
via `--trust-host` (sets `CCLAW_TRUST_LEVEL=host`). Sandbox on/off is derived
...
- The level must never gate a capability. If a change makes tool/secret
  access depend on `trust_level`, it belongs on `grants` instead.
...
`test_sandbox_mounts.c`). The level→bundle mapping itself
(`sandbox_policy_from_trust`) has no direct unit test yet.
