...
static void setup_shell_tool(AgentSetup *setup, sqlite3 *db, const Config *cfg,
    ...
    if (!trust_level) {
        ...
        if (sqlite3_prepare_v2(db, "SELECT trust_level FROM agents WHERE name=?", -1, &tl_stmt, NULL) == SQLITE_OK) {
    ...
    if (shell_entry && shell_entry->user_data) {
        ...
        } else if (trust_level && (strcmp(trust_level, "trusted") == 0 || strcmp(trust_level, "bootstrap") == 0)) {
            sc->env_mode = 0; sc->net_mode = 0; sc->mount_cwd = 1; sc->workspace_ro = 0;
            sc->rlimits.nproc = 0; sc->rlimits.as_mb = 0; sc->rlimits.cpu_sec = 0;
...
static void setup_file_tools(AgentSetup *setup, const Config *cfg, int64_t session_id) {
    char tmp_dir[64];
...
static void setup_js_tools(AgentSetup *setup, sqlite3 *db, int64_t session_id,
                           const Config *cfg, char **allowed_hosts, size_t allowed_hosts_count) {
    ...
    if (cfg->workspace) {
        ...
        if (ext_paths && ext_count > 0) {
            extension_load(ext_paths, ext_count, setup->js_rt, &setup->reg, cfg, &setup->ext_ctx);
            extension_list_free(ext_paths, ext_count);
        }
    }
    if (setup->js_rt) js_runtime_set_registry(setup->js_rt, &setup->reg);
}

static void setup_daemon_tools(AgentSetup *setup, sqlite3 *db, int64_t session_id,
                               const char *agent_name) {
...
int agent_setup_init(AgentSetup *setup, sqlite3 *db, int64_t session_id,
    ...
    setup_shell_tool(setup, db, cfg, agent_name);
    setup_file_tools(setup, cfg, session_id);
    setup_js_tools(setup, db, session_id, cfg, allowed_hosts, allowed_hosts_count);
    ...
    if (mode == AGENT_SETUP_DAEMON)
        setup_daemon_tools(setup, db, session_id, agent_name);
