...
void print_usage(void) {
    printf("usage: cclaw [options]\n"
           ...
           "                                   rename an agent everywhere (DB + workspace dir).\n"
           "                                   Takes a quiesce lease, drains in-flight turns\n"
           "                                   (default 60s), then tells the agent. Works with\n"
           "                                   the daemon running\n"
           ...
           "       cclaw update [--check] [--tag vX.Y.Z]\n"
           "                                   install a newer tagged release. Refuses a build\n"
           "                                   that could not open this database (schema\n"
           "                                   patches are forward-only, so that is not\n"
           "                                   undoable by swapping back); snapshots the DB,\n"
           "                                   swaps atomically, and reverts if the daemon\n"
           "                                   does not come back. Restarts only if\n"
           "                                   update.restart_command is set — otherwise the\n"
           "                                   running daemon is left alone and picks the new\n"
           "                                   build up at your next restart.\n"
           "                                   --check exits 10 when an update is available\n"
