...
static const struct { int version; const char *sql; int (*fn)(sqlite3 *); } schema_patches[] = {
    ...
    /* v48: provider wire-format M1 (plan/projects/provider-wire-format-refactor.md).
...
static int db_schema_upgrade(sqlite3 *db) {
    ...
    for (int i = 0; i < n_patches; i++) {
        ...
        int rc = schema_patches[i].fn
                     ? (schema_patches[i].fn(db) == 0 ? SQLITE_OK : SQLITE_ERROR)
                     : sqlite3_exec(db, schema_patches[i].sql, NULL, NULL, &err);
        if (rc != SQLITE_OK) {
...
/* "In flight" for the session_max_active launch gate = non-idle OR idle with
 * an unconsumed inbox row. The inbox arm is what makes the cap batch-aware: a
 ...
 * sail past the cap the same calls spread over iterations are refused by.
...
int64_t db_cost_last_24h(sqlite3 *db) {
    ...
    return db_scalar_i64(db,
        ...
        " WHERE created_at > unixepoch()-86400;", 0, 0);
}
