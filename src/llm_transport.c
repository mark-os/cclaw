...
char *llm_build_auth_header(Arena *a, const Config *cfg) {
    ...
    if (cfg->provider.endpoint_type == ENDPOINT_GEMINI) {
        size_t size = 16 + key_len + 1;
        char *h = arena_alloc(a, size);
        if (!h) return NULL;
        snprintf(h, size, "x-goog-api-key: %s", cfg->provider.api_key ? cfg->provider.api_key : "");
        return h;
    }
    size_t size = 22 + key_len + 1;
    char *h = arena_alloc(a, size);
    if (!h) return NULL;
    snprintf(h, size, "Authorization: Bearer %s", cfg->provider.api_key ? cfg->provider.api_key : "");
    return h;
}
