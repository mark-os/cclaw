...
static void test_schema_patch_v47(void) {
    ...
    assert(sqlite3_exec(db,
        ...
        /* v48's columns go too — this DB is walked from 46 all the way up. */
...
/* v48 (provider wire-format M1): the two observability/config columns land,
...
static void test_schema_patch_v48(void) {
    const char *path = "/tmp/test_cclaw_db_v48.sqlite";
    test_db_clean(path);
    sqlite3 *db = test_db_open(path);
    assert(db != NULL);
