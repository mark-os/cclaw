...
#define DB_PATH "/tmp/test_e2e_agent.db"

static const char *MOCK_RESPONSE =
    "{\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"I am here\"},"
    "\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":10,\"completion_tokens\":3}}";

int main(void) {
    alarm(10);
    printf("test_e2e_agent:\n");
    ...
    for (int i = 0; i < count; i++) {
        if (branch[i].message.role == ROLE_ASSISTANT &&
            branch[i].message.content && strstr(branch[i].message.content, "I am here"))
            found = 1;
    }
    assert(found);
