/* T284: e2e non-streaming mode
 * Verifies: CCLAW_STREAM=0 omits "stream":true from request,
 * response is full JSON (not SSE), no ASAN/UBSan, exit 0.
 * Requires OPENROUTER_API_KEY. */
#define _POSIX_C_SOURCE 200809L
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/wait.h>

#define BINARY "./build/cclaw"

static int tests_run = 0;
static int tests_passed = 0;

#define TEST(name) do { tests_run++; printf("  %s... ", #name); } while(0)
#define PASS() do { tests_passed++; printf("PASS\n"); } while(0)
#define FAIL(msg) do { printf("FAIL: %s\n", msg); return; } while(0)
#define SKIP(msg) do { tests_passed++; printf("SKIP: %s\n", msg); return; } while(0)

static int has_sanitizer_errors(const char *path) {
    FILE *f = fopen(path, "r");
    if (!f) return 0;
    char line[4096];
    while (fgets(line, sizeof(line), f)) {
        if (strstr(line, "ERROR: AddressSanitizer") ||
            strstr(line, "ERROR: LeakSanitizer") ||
            strstr(line, "runtime error:"))
        { fclose(f); return 1; }
    }
    fclose(f);
    return 0;
}

static int file_contains(const char *path, const char *needle) {
    FILE *f = fopen(path, "r");
    if (!f) return 0;
    char line[65536];
    while (fgets(line, sizeof(line), f)) {
        if (strstr(line, needle)) { fclose(f); return 1; }
    }
    fclose(f);
    return 0;
}

static void test_nonstream_mode(void) {
    TEST(nonstream_no_stream_true_in_request);
    if (!getenv("OPENROUTER_API_KEY")) SKIP("OPENROUTER_API_KEY not set");

    const char *out = "/tmp/cclaw_e2e_t284_nonstream.txt";
    char cmd[1024];
    snprintf(cmd, sizeof(cmd),
        "CCLAW_STREAM=0 timeout 120 %s --trust-host --new --log-level=trace "
        "-p \"What is 3+5? Answer with just the number.\" > %s 2>&1",
        BINARY, out);
    int rc = system(cmd);
    rc = WIFEXITED(rc) ? WEXITSTATUS(rc) : -1;

    if (has_sanitizer_errors(out)) FAIL("sanitizer errors detected");
    if (rc != 0) FAIL("non-zero exit code");
    /* Request must NOT contain "stream":true (non-streaming omits the field) */
    if (file_contains(out, "\"stream\":true"))
        FAIL("request contains \"stream\":true — should be absent when CCLAW_STREAM=0");
    /* Response must be full JSON (RESP line with "choices") not SSE chunks */
    if (!file_contains(out, "RESP status=200"))
        FAIL("no RESP line — expected full JSON response");
    /* Output should contain the answer */
    if (!file_contains(out, "8")) FAIL("expected '8' in output");

    PASS();
}

int main(void) {
    printf("--- test_e2e_nonstream (T284) ---\n");
    test_nonstream_mode();
    printf("%d/%d passed\n", tests_passed, tests_run);
    return tests_passed == tests_run ? 0 : 1;
}
