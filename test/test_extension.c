...
static void test_discover_files(void) {
    ...
    snprintf(p1, sizeof(p1), "%s/beta.js", ext_dir);
    snprintf(p2, sizeof(p2), "%s/alpha.js", ext_dir);
    snprintf(p3, sizeof(p3), "%s/readme.txt", ext_dir); /* not .js */
    write_file(p1, "// beta");
    write_file(p2, "// alpha");
    ...
    if (!strstr(paths[0], "alpha.js")) FAIL("first should be alpha.js");
    if (!strstr(paths[1], "beta.js")) FAIL("second should be beta.js");
    extension_list_free(paths, count);
...
static void test_discover_subdir_index(void) {
    ...
    char idx[512];
    snprintf(idx, sizeof(idx), "%s/index.js", sub_dir);
    write_file(idx, "// plugin");

    size_t count = 0;
    char **paths = extension_discover(ws, &count);
    if (count != 1) { printf("got %zu ", count); FAIL("expected 1"); }
    if (!strstr(paths[0], "myplugin/index.js")) FAIL("should be myplugin/index.js");
    extension_list_free(paths, count);
...
static void test_load_success(void) {
    ...
    char p1[512];
    snprintf(p1, sizeof(p1), "%s/good.js", ext_dir);
    ...
    int loaded = extension_load(paths, count, rt, NULL, &cfg);
    extension_list_free(paths, count);
...
static void test_load_throw_skips(void) {
    ...
    snprintf(p2, sizeof(p2), "%s/good.js", ext_dir);
