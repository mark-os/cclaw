...
static void test_rollback_missing_session(void) {
    ...
    assert(db);
    int64_t sid = session_create(db, "rollback_miss");
    assert(sid > 0);
...
static void test_rollback_mid_consumption(void) {
    sqlite3 *db = db_open(DB_PATH);
    assert(db);
    int64_t sid = session_create(db, "rollback_mid");
    assert(sid > 0);
    ...
    for (int threshold = 10; threshold < 500; threshold += 5) {
        ...
        if (consumed == 5) {
            ...
            unlink(DB_PATH);
            db = db_open(DB_PATH);
            sid = session_create(db, "rollback_mid");
            for (int i = 0; i < 5; i++) {
                char buf[32];
                snprintf(buf, sizeof(buf), "item_%d", i);
                inbox_insert(db, sid, "src", buf);
...
static void test_no_double_consume_after_success(void) {
    sqlite3 *db = db_open(DB_PATH);
    assert(db);
    int64_t sid = session_create(db, "no_double");
    assert(sid > 0);
