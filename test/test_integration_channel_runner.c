...
static void write_test_js(void) {
    ...
    fprintf(f,
        ...
        "}\n"
        "function onInit() {\n"
        "  token = cclaw.getConfig('bot_token');\n"
        "  base = cclaw.getConfig('base_url');\n"
        "  cclaw.log('init ok');\n"
        "  return {poll: shape()};\n"
        "}\n"
        ...
        "        cclaw.emit('message', JSON.stringify({channel_id: '' + msg.chat.id, text: msg.text}));\n"
        "      }\n"
        "    }\n"
        "  }\n"
        "  return {poll: shape()};\n"
        ...
        "  cclaw.send({method: 'POST', url: base + '/bot' + token + '/sendMessage',\n"
        ...
        "  cclaw.emit('message', JSON.stringify({channel_id: 'uds', text: req.body}));\n"
