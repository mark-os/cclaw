...
static const char *stream_to_mock(sqlite3 *db, int64_t sid, Config *cfg,
                                  ToolSchema *tools, size_t tool_count, int port) {
    ContextPlan plan;
...
static void test_wire_escaped_content(void) {
    ...
    const char *body = stream_to_mock(db, sid, &cfg, NULL, 0, port);
    if (!body) { db_close(db); mock_server_stop(); FAIL("no body captured"); }

    /* Parse as JSON — must be valid */
    cJSON *root = cJSON_Parse(body);
    if (!root) { db_close(db); mock_server_stop(); FAIL("invalid JSON"); }

    cJSON *messages = cJSON_GetObjectItem(root, "messages");
    if (!messages || cJSON_GetArraySize(messages) < 1) {
        cJSON_Delete(root); db_close(db); mock_server_stop();
        FAIL("no messages");
    }
...
static void test_wire_unicode_content(void) {
    ...
    const char *body = stream_to_mock(db, sid, &cfg, NULL, 0, port);
    if (!body) { db_close(db); mock_server_stop(); FAIL("no body"); }
...
static void test_wire_tool_calls_format(void) {
    ...
    cJSON *root = cJSON_Parse(body);
    if (!root) { db_close(db); mock_server_stop(); FAIL("invalid JSON from wire"); }
    ...
    if (strcmp(cJSON_GetObjectItem(parsed_args0, "path")->valuestring, "/tmp/x.txt") != 0) {
        cJSON_Delete(parsed_args0); cJSON_Delete(root); db_close(db); mock_server_stop();
