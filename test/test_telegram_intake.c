...
static void test_tg_intake_inserts_to_inbox(void) {
    ...
    assert(db);
    int64_t sid = session_create(db, "tg_intake");
    assert(sid > 0);
...
static void test_tg_intake_acquire_and_consume(void) {
    sqlite3 *db = db_open(DB_PATH);
    assert(db);
    int64_t sid = session_create(db, "tg_acquire");
    assert(sid > 0);
...
static void test_tg_intake_lock_contention(void) {
    sqlite3 *db = db_open(DB_PATH);
    assert(db);
    int64_t sid = session_create(db, "tg_contention");
    assert(sid > 0);
...
static void test_tg_intake_batch_consume(void) {
    sqlite3 *db = db_open(DB_PATH);
    assert(db);
    int64_t sid = session_create(db, "tg_batch");
    assert(sid > 0);
